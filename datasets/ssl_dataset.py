...
class Emotion_SSL_Dataset:
    ...
    def get_data(self, svhn_extra=True):
        ...
        if self.data_dir is None:
            ...
            exit(-1)
        base_dataset = torchvision.datasets.ImageFolder(self.data_dir)
        # data, targets = base_dataset.data, base_dataset.targets
        imgs = np.array(base_dataset.imgs)
        img_path, img_class = imgs[:, 0], imgs[:, 1]
        img_class = [int(element) for element in img_class]
